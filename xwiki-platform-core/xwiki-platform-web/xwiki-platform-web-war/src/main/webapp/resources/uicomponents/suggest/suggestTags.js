 ...
 */
define('xwiki-suggestTags', ['jquery', 'xwiki-entityReference', 'xwiki-selectize'], function ($, XWiki) {
    function getSelectizeOptions(select)
    {
        let wiki = new XWiki.WikiReference(XWiki.currentWiki);
        const loadURL = [
            XWiki.contextPath, 'rest',
            'wikis', encodeURIComponent(wiki),
            'tags'
        ].join('/');
            ...
            return async (text, callback) => {
                try {
                    ...
                    if (Array.isArray(response?.tags)) {
                        callback(response.tags.map(getSuggestion));
                    } else {
                        callback([]);
                    }
                } catch {
